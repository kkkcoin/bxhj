...
static CBigNum bnStartLimit(~uint256(0) >> 31);
...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 60; // stake age of full weight
unsigned int nStakeTargetSpacing = 4 * 60;
const int64 nChainStartTime = 1430255654; 
...
const string strMessageMagic = "Kraken Signed Message:\n";
...
static const int64 nTargetTimespan = 0.25 * 24 * 60 * 60;  // 6-hour
static const int64 nTargetSpacingWorkMax = 5 * nStakeTargetSpacing;
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    CBigNum bnTargetLimit = bnProofOfWorkLimit;
		...
		return bnStartLimit.GetCompact();
    ...
    if(fProofOfStake)
    {
        ...
        else
        {
            if(pindexLast->nHeight + 1 > 150)
                bnTargetLimit = bnProofOfStakeLimit;
            else if(pindexLast->nHeight + 1 > 140)
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Kraken miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Kraken
    bool fStrictPayToScriptHash = true; // Always active in Kraken
    ...
    // ppcoin: fees are not collected by miners as in Kraken
...
bool CBlock::AcceptBlock()
{
    ...
    if (nVersion < 3 && ((!fTestNet && nHeight > 140) || (fTestNet && nHeight > 0)))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Kraken", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Unus pro omnibus, omnes pro uno";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:Kraken genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x0721aab0cb42ce3f53c9e5dfa6d59d36e0bd2c261381925071db611e749d2d2f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe3, 0xd1, 0xa1, 0xf5 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("KrakenMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KrakenMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("KrakenMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadKrakenMiner(void* parg);

static bool fGenerateKraken = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void KrakenMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("Kraken-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateKraken || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateKraken) && !fProofOfStake)
                return;
        }
        ...
        printf("Running KrakenMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadKrakenMiner(void* parg)
{
    ...
    try
    {
        ...
        KrakenMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadKrakenMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadKrakenMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadKrakenMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateKraken(bool fGenerate, CWallet* pwallet)
{
    fGenerateKraken = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateKraken = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d KrakenMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadKrakenMiner, pwallet))
                printf("Error: NewThread(ThreadKrakenMiner) failed\n");
            Sleep(10);
        }
    }
}
